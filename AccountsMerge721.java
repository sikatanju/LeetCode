    ...
    public List<List<String>> accountsMerge(List<List<String>> accounts) {
        ...
        for (int i=0; i<len; i++)   {
            if (!visited[i])    {
                ...
                Set<String> set = new HashSet<>(accounts.get(i).subList(1, accounts.get(i).size()));
                for (int j=0; j<len; j++)   {
                    var currName = name.equals(accounts.get(j).getFirst());
                    if (!visited[j] && currName) {
                        List<String> list = new ArrayList<>(accounts.get(j).subList(1, accounts.get(j).size()));
                        for (var str: list) {
                            if (set.contains(str))  {
                                visited[j] = true;
                                break;
